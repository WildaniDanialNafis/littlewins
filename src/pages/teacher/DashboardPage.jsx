...
const getStudentName = (report, students) => {
  if (report?.student_name) {
    return report.student_name;
  }

  if (report?.student?.full_name) {
  ...
  const student = students.find(
    (item) => Number(item.id) === Number(report?.student_id),
  );
...
const DashboardPage = () => {
  ...
  const currentTeacher = useMemo(() => {
    ...
    return {
      ...
      name: user.profile?.full_name?.trim() || user.full_name?.trim() || "Guru",
    };
  }, [user]);
  ...
  const teacherReports = useMemo(() => {
    ...
    const filteredReports = reports.filter(
      (report) => Number(report.teacher_id) === currentTeacher.id,
    );

    /*
     * Preserve existing behavior:
     * if no report belongs to the current teacher,
     * use the available reports instead of showing an empty dashboard.
     */
    return filteredReports.length > 0 ? filteredReports : reports;
  }, [reports, currentTeacher]);

  const latestReport = useMemo(
    () => getLatestReport(teacherReports),
    [teacherReports],
  );
  ...
  if (hasError) {
    return (
      <PageContainer title="Dashboard Guru" subtitle="Gagal memuat data.">
        <ContentBlock>
  ...
  return (
    <PageContainer
      ...
      subtitle={
        currentTeacher?.name
          ? `Selamat datang kembali, ${currentTeacher.name}.`
      ...
      <div className="space-y-8">
        ...
        <ContentBlock>
          <header className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              <ActionLink
                to={ROUTES.teacher.settings || ROUTES.settings || "#"}
                variant="secondary"
        ...
        <section aria-label="Laporan terbaru">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <SectionTitle
              ...
              description="Laporan terakhir yang Anda buat."
            />
            ...
            {!latestReportData ? (
              <ContentBlock>
                <EmptyState
                  ...
                  description="Anda belum membuat laporan belajar. Buat laporan pertama untuk mulai mencatat perkembangan siswa."
                  action={
                    <ActionLink to={ROUTES.teacher.reportNew} variant="primary">
                      <PlusIcon className="h-4 w-4" aria-hidden="true" />
              ...
              <ContentBlock>
                <article>
                  ...
                  <div className="mt-5 border-t border-border pt-4">
                    <ActionLink
                      to={ROUTES.teacher.reportDetail(latestReportData.id)}
                      variant="primary"
                      className="w-full sm:w-auto"
                    >
